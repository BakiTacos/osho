...
import { 
  ...
  TrendingDown, Receipt, History, Pencil, Landmark 
} from "lucide-react";
...
export default function PembayaranPage() {
  ...
  // 🚀 TAB STATE (Sesuai Desain Baru)
  const [activeTab, setActiveTab] = useState<"nota" | "opex" | "tarik">("nota");
  ...
  return (
    <div className="text-[#1E293B] ml-0 lg:ml-72 min-h-screen bg-[#F8F9FB] transition-all duration-300 pb-20">
      ...
      {/* ========================================== */}
      {/* 🚀 1. HEADER (Desain Baru Bersih & Minimalis) */}
      {/* ========================================== */}
      ...
      {/* ========================================== */}
      {/* 🚀 2. FILTER PILL (Desain Lebar dengan Divider) */}
      {/* ========================================== */}
            ...
            <div className="flex items-center w-full sm:w-auto pl-1 sm:pl-2">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <select 
                  ...
                  className="flex-1 sm:flex-none bg-slate-50 border border-slate-100 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black uppercase px-3 py-2 sm:py-2.5 text-[#0047AB] outline-none hover:bg-slate-100 cursor-pointer transition-colors"
                >
      ...
      {/* ========================================== */}
      {/* 🚀 3. TAB MENU (Teks Bersih dengan Underline) */}
      {/* ========================================== */}
      ...
      {/* ============================================== */}
      {/* 🚀 KONTEN HALAMAN 1: NOTA SUPPLIER */}
      {/* ============================================== */}
      {activeTab === "nota" && (
        <div className="px-4 sm:px-10 mt-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            <div className="bg-white p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-black text-emerald-600 uppercase mb-2">Total Nota Dibayar</p>
                <h3 className="text-2xl font-black text-[#0F172A] truncate">Rp {totalPaidInvoices.toLocaleString('id-ID')}</h3>
              </div>
            </div>
            <div className="bg-white p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-black text-red-500 uppercase mb-2">Nota Belum Bayar</p>
                <h3 className="text-2xl font-black text-[#0F172A] truncate">Rp {totalUnpaidInvoices.toLocaleString('id-ID')}</h3>
              </div>
            </div>
          ...
          <div className="bg-white rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm overflow-hidden min-h-[350px]">
            <div className="p-5 sm:p-8 border-b border-slate-50 flex justify-between items-center text-[10px] sm:text-xs font-black uppercase text-[#94A3B8] tracking-widest">
                      ...
                      <td className="px-5 py-4 sm:px-8 sm:py-5 text-right relative">
                        ...
                        {activeMenuId === inv.id && (
                          <div className="absolute right-8 top-10 w-32 bg-white border rounded-xl shadow-xl z-20 py-1 overflow-hidden">
                            <button onClick={() => { 
                              setInvoiceForm({ id: inv.id, noNota: inv.noNota, supplier: inv.supplier, dueDate: inv.dueDate || getLocalDateString(), status: inv.status }); 
                              setInvoiceItems(inv.items); setIsInvoiceModalOpen(true); setActiveMenuId(null); 
                            }} className="w-full flex items-center gap-2.5 px-4 py-3 text-[9px] sm:text-[10px] font-black text-[#0F172A] hover:bg-slate-50"><Edit3 size={12} className="text-[#0047AB]"/> EDIT</button>
      ...
      {/* ============================================== */}
      {/* 🚀 KONTEN HALAMAN 2: BIAYA OPERASIONAL */}
      {/* ============================================== */}
      {activeTab === "opex" && (
        <div className="px-4 sm:px-10 mt-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            <div className="bg-white p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-black text-orange-400 uppercase mb-2">Total Biaya Operasional</p>
                <h3 className="text-2xl font-black text-[#0F172A] truncate">Rp {totalOpex.toLocaleString('id-ID')}</h3>
              </div>
            </div>
               ...
               <div className="space-y-2 max-h-[80px] overflow-y-auto no-scrollbar">
                {Object.entries(payerStats).map(([payer, amount]) => (
                  <div key={payer} className="flex justify-between text-[10px] font-bold text-slate-500 uppercase">
                    ...
                    <span className="text-[#0F172A] shrink-0 font-black">Rp {(amount as number).toLocaleString('id-ID')}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm p-6 sm:p-8 min-h-[350px]">
             <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-[#94A3B8] mb-6 sm:mb-8 flex items-center gap-2">
                  ...
                  <div key={exp.id} className="flex justify-between items-center p-4 sm:p-5 bg-slate-50 rounded-2xl sm:rounded-3xl border border-slate-100 group transition-all">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <p className="text-sm sm:text-base font-black text-[#0F172A] leading-tight truncate">{exp.category}</p>
                          <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-md text-[8px] font-black text-slate-400 uppercase tracking-tighter shrink-0">{exp.paidBy || 'KEVIN'}</span>
                          {displayDate && <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">• {displayDate}</span>}
                        </div>
                    ...
                    <div className="flex flex-col items-end shrink-0 ml-4">
                      ...
                      <div className="flex items-center mt-2 gap-3 opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => {
                          setExpenseForm({ id: exp.id, category: exp.category, description: exp.description, amount: exp.amount.toString(), paidBy: exp.paidBy || '', date: exp.date || getLocalDateString() });
                          setIsExpenseModalOpen(true);
                        }} className="p-1.5 text-slate-400 hover:text-[#0047AB] flex items-center gap-1.5"><Pencil size={14}/> <span className="text-[9px] font-black uppercase hidden sm:block">Edit</span></button>
      ...
      {/* ============================================== */}
      {/* 🚀 KONTEN HALAMAN 3: PENARIKAN SALDO MP */}
      {/* ============================================== */}
      {activeTab === "tarik" && (
        <div className="px-4 sm:px-10 mt-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            <div className="bg-white p-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-black text-emerald-600 uppercase mb-2">Total Saldo Ditarik</p>
                <h3 className="text-2xl font-black text-[#0F172A] truncate">Rp {totalWithdrawal.toLocaleString('id-ID')}</h3>
              </div>
            </div>
               ...
               <div className="space-y-2 max-h-[80px] overflow-y-auto no-scrollbar">
                {Object.keys(platformStats).map(p => (
                  <div key={p} className="flex justify-between text-[10px] font-bold text-slate-500 uppercase">
                    ...
                    <span className="text-[#0047AB] shrink-0 font-black">Rp {platformStats[p].toLocaleString('id-ID')}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button onClick={() => { setWithdrawForm({ id: null, platform: 'Shopee', amount: '', date: getLocalDateString() }); setIsWithdrawModalOpen(true); }} className="w-full flex-1 flex items-center justify-center gap-2 bg-emerald-600 text-white rounded-[16px] sm:rounded-[20px] font-black text-xs uppercase shadow-md shadow-emerald-100 hover:scale-[1.02] transition-all">
                <Wallet size={18} /> <span>Tarik Saldo Baru</span>
              </button>
              <button onClick={() => setIsHistoryModalOpen(true)} className="w-full flex-1 flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-600 rounded-[16px] sm:rounded-[20px] font-black text-xs uppercase hover:bg-slate-50 transition-all">
                <History size={18} /> <span>Lihat Riwayat Lengkap</span>
              </button>
            </div>
          </div>
