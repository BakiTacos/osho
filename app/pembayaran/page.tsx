...
export default function PembayaranPage() {
  ...
  const handleSaveWithdraw = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, `users/${currentUser?.uid}/withdrawals`), { ...withdrawForm, amount: Number(withdrawForm.amount), status: 'Berhasil', editCount: 0, createdAt: serverTimestamp() });
  ...
  const handleSaveInvoice = async (e: React.FormEvent) => {
    ...
    if (hasInvalid) return alert("Qty dan Harga harus lebih besar dari 0");
  ...
  const handleDeleteInvoice = async (inv: any) => {
    if (!confirm("Hapus nota? Stok akan dikurangi kembali.")) return;
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, `users/${currentUser?.uid}/expenses`), { ...expenseForm, amount: Number(expenseForm.amount), createdAt: serverTimestamp() });
    ...
    <div className="text-[#1E293B] ml-0 lg:ml-72 min-h-screen bg-[#F8F9FB] transition-all duration-300 pb-20">
      ...
      <div className="px-4 sm:px-10 mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-6 rounded-[28px] border border-[#F1F5F9] shadow-sm relative group">
          ...
          <div className="mt-3 space-y-1">
            {Object.keys(platformStats).map(p => (
              <div key={p} className="flex justify-between text-[9px] font-bold text-slate-400">
                <span>{p}</span><span className="text-[#0047AB]">Rp {platformStats[p].toLocaleString('id-ID')}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => setIsWithdrawModalOpen(true)} className="flex-1 bg-white border-2 border-slate-50 text-[#0047AB] rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase hover:bg-blue-50 transition-all"><Wallet size={14} /> Tarik Saldo</button>
          <button onClick={() => setIsInvoiceModalOpen(true)} className="flex-1 bg-[#0047AB] text-white rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase shadow-lg shadow-blue-100 hover:scale-[1.02] transition-all"><PlusCircle size={14} /> Nota Baru</button>
          <button onClick={() => setIsExpenseModalOpen(true)} className="flex-1 bg-orange-500 text-white rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase shadow-lg shadow-orange-100 hover:scale-[1.02] transition-all"><TrendingDown size={14} /> Input Opex</button>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-[#F8F9FB] text-[9px] font-black text-[#94A3B8] uppercase tracking-widest">
                <tr><th className="px-8 py-5">Nomor Nota / Supplier</th><th className="px-6 py-5">Status</th><th className="px-6 py-5 text-right">Tagihan</th><th className="px-8 py-5 text-right">Aksi</th></tr>
              </thead>
                ...
                {filteredInvoices.map((inv) => (
                  <tr key={inv.id} className="group hover:bg-slate-50/50 transition-all text-sm font-bold">
                    ...
                    <td className="px-6 py-5">
                      <button onClick={async () => { const next = inv.status === 'TERBAYAR' ? 'BELUM BAYAR' : 'TERBAYAR'; await updateDoc(doc(db, `users/${currentUser?.uid}/supplier_invoices`, inv.id), { status: next }); }}
                        className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${inv.status === 'TERBAYAR' ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}>{inv.status}</button>
                    </td>
                    ...
                    <td className="px-8 py-5 text-right relative">
                      ...
                      {activeMenuId === inv.id && (
                        <div className="absolute right-10 top-12 w-36 bg-white border rounded-xl shadow-xl z-20 py-1 overflow-hidden">
                          <button onClick={() => { setEditingInvoiceId(inv.id); setInvoiceForm({ noNota: inv.noNota, supplier: inv.supplier, dueDate: inv.dueDate || '', status: inv.status }); setInvoiceItems(inv.items); setIsInvoiceModalOpen(true); setActiveMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-[10px] font-black text-slate-600 hover:bg-slate-50"><Edit3 size={14}/> EDIT</button>
            ...
            {filteredExpenses.map((exp) => (
              <div key={exp.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-[11px] font-black text-[#0F172A] leading-tight">{exp.category}</p><p className="text-[9px] font-bold text-slate-400 uppercase">{exp.description}</p></div>
                </div>
      ...
      {/* MODAL: NOTA & RESTOCK (FIXED HEADERS & PRICE BOX) */}
      {isInvoiceModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-5xl rounded-[40px] p-8 max-h-[90vh] overflow-y-auto shadow-2xl relative no-scrollbar">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={resetInvoiceForm} className="p-2 text-slate-400 hover:bg-slate-50 rounded-full"><X/></button>
            </div>
            ...
            <form onSubmit={handleSaveInvoice} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input required value={invoiceForm.noNota} className="w-full bg-slate-50 rounded-2xl py-4 px-6 font-bold text-sm border-none focus:ring-2 focus:ring-[#0047AB]" placeholder="Contoh: INV/2024/001" onChange={e => setInvoiceForm({...invoiceForm, noNota: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Nama Supplier</label>
                  <input required value={invoiceForm.supplier} className="w-full bg-slate-50 rounded-2xl py-4 px-6 font-bold text-sm border-none focus:ring-2 focus:ring-[#0047AB]" placeholder="Nama Toko / Distributor" onChange={e => setInvoiceForm({...invoiceForm, supplier: e.target.value})} />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center px-2">
                  ...
                  <button type="button" onClick={() => setInvoiceItems([...invoiceItems, {sku:'', name:'', qty:1, price:0, unit:'lusin'}])} className="flex items-center gap-2 text-[9px] font-black bg-blue-50 text-[#0047AB] px-4 py-2 rounded-xl hover:bg-blue-100 transition-all"><Plus size={12}/> TAMBAH ITEM</button>
                </div>
                ...
                <div className="space-y-2">
                  {invoiceItems.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-3 bg-slate-50/50 p-3 rounded-2xl items-center border border-slate-100">
                      <input required className="col-span-2 bg-white py-3 px-4 rounded-xl text-xs font-bold border-none shadow-sm focus:ring-2 focus:ring-blue-200" placeholder="SKU" value={item.sku} onChange={e => {
                      ...
                      <select className="col-span-2 bg-white py-3 px-2 rounded-xl text-xs font-black text-[#0047AB] border-none shadow-sm cursor-pointer" value={item.unit} onChange={e => { const newItems = [...invoiceItems]; newItems[idx].unit = e.target.value; setInvoiceItems(newItems); }}>
                        <option value="lusin">Lusin (x12)</option>
                        <option value="pcs">Pcs (Satuan)</option>
                      </select>
                      <input type="number" required min="1" className="col-span-2 bg-white py-3 px-4 rounded-xl text-xs font-bold text-center border-none shadow-sm focus:ring-2 focus:ring-blue-200" value={item.qty} onChange={e => { const newItems = [...invoiceItems]; newItems[idx].qty = Number(e.target.value); setInvoiceItems(newItems); }}/>
                      
                      {/* --- BOX HARGA (FIXED) --- */}
                      <input type="number" required min="1" className="col-span-2 bg-white py-3 px-4 rounded-xl text-xs font-black text-[#0047AB] border-none shadow-sm focus:ring-2 focus:ring-blue-200" placeholder="Rp" value={item.price || ''} onChange={e => { const newItems = [...invoiceItems]; newItems[idx].price = Number(e.target.value); setInvoiceItems(newItems); }}/>
              ...
              <div className="flex justify-between items-center pt-6 border-t border-slate-50">
                <div className="space-y-1">
                  ...
                  <div className="text-2xl font-black text-[#0F172A]">Rp {invoiceItems.reduce((a,b) => a + (b.qty * b.price), 0).toLocaleString('id-ID')}</div>
                </div>
                <div className="flex gap-3">
                  <button type="button" onClick={resetInvoiceForm} className="px-8 py-4 bg-slate-100 rounded-2xl font-black text-[10px] uppercase hover:bg-slate-200 transition-all">Batal</button>
      ...
      {isExpenseModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[32px] p-8 animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-8">
               <h2 className="text-2xl font-black tracking-tighter text-orange-500">Input Opex</h2>
