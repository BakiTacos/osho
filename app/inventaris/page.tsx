...
import { 
  ...
  Edit2, Trash2, Check, ArrowUpDown, Filter
} from "lucide-react";
    ...
    <div className="text-[#1E293B] ml-0 lg:ml-72 min-h-screen bg-[#F8F9FB] transition-all duration-300">
      ...
      <div className="px-4 sm:px-10 pt-6 sm:pt-8 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-6 text-[#64748B]">
          <Bell size={20} className="hidden sm:block cursor-pointer hover:text-[#0F172A]" />
      ...
      <div className="px-4 sm:px-10 mt-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-4xl font-black text-[#0F172A] tracking-tighter leading-none">Inventaris</h1>
          <p className="text-[#64748B] mt-2 text-sm font-medium">Kelola stok dan katalog produk Anda.</p>
        </div>
        <Link href="/inventaris/tambah">
          <button className="flex items-center space-x-2 bg-[#0047AB] text-white px-6 py-3 rounded-xl font-black text-sm shadow-xl shadow-blue-100 hover:scale-105 transition-all active:scale-95">
      ...
      <div className="px-4 sm:px-10 mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-7 rounded-[28px] border border-[#F1F5F9] shadow-sm relative overflow-visible">
          <div className="flex justify-between items-start mb-6">
            ...
            <Tooltip text="Total produk unik (SKU)."><HelpCircle size={14} className="opacity-20 cursor-help" /></Tooltip>
          </div>
        ...
        <div className="bg-white p-7 rounded-[28px] border border-[#F1F5F9] shadow-sm relative overflow-visible">
          <div className="flex justify-between items-start mb-6">
            ...
            <Tooltip text="Stok di bawah 10 unit."><HelpCircle size={14} className="opacity-20 cursor-help" /></Tooltip>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="bg-[#E11D48] text-white text-[9px] font-black px-2 py-0.5 rounded uppercase">Warning</span>
          </div>
        ...
        <div className="bg-white p-7 rounded-[28px] border border-[#F1F5F9] shadow-sm relative overflow-visible">
          <div className="flex justify-between items-start mb-6">
            ...
            <Tooltip text="Produk dengan stok nol."><HelpCircle size={14} className="opacity-20 cursor-help" /></Tooltip>
          </div>
          <p className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-1">Out of Stock</p>
      ...
      <div className="px-4 sm:px-10 py-8">
        <div className="bg-white rounded-[28px] border border-[#F1F5F9] shadow-sm overflow-visible">
            ...
            <table className="w-full text-left">
              <thead className="bg-[#F8F9FB] border-b border-[#F1F5F9]">
                <tr className="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">
                  ...
                  <th className="px-6 py-5 text-right">Modal (HPP)</th>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center space-x-4">
                        <div className="w-10 h-10 rounded-lg bg-slate-100 overflow-hidden border border-slate-50">
                    ...
                    <td className="px-6 py-5">
                      <span className="px-2.5 py-1 bg-slate-100 text-[#0047AB] text-[9px] font-black rounded uppercase tracking-tighter">
                        {p.category}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-5 text-center">
                      {editingStockId === p.id ? (
                        <div className="flex items-center justify-center space-x-1">
                          ...
                          />
                          <button onClick={() => handleUpdateStock(p.id)} className="text-green-600 hover:scale-110 transition-all"><Check size={16}/></button>
                        </div>
                      ) : (
                        <div 
                    ...
                    <td className="px-8 py-5 text-right relative">
                      <button 
                        ...
                        className="p-2 hover:bg-slate-100 rounded-lg text-[#94A3B8] hover:text-[#0F172A] transition-all"
                      >
                      ...
                      {activeMenuId === p.id && (
                        <>
                          ...
                          <div className="absolute right-10 top-12 w-40 bg-white border border-[#F1F5F9] rounded-xl shadow-2xl z-[110] overflow-hidden py-1 animate-in fade-in zoom-in duration-200">
                            <Link href={`/inventaris/edit/${p.id}`} className="flex items-center space-x-3 px-4 py-3 text-xs font-black text-[#64748B] hover:bg-[#F8F9FB] hover:text-[#0047AB] transition-colors">
                              <Edit2 size={14} /> <span>Edit Produk</span>
                            </Link>
                            <button 
                              onClick={() => handleDelete(p.id)}
                              className="w-full flex items-center space-x-3 px-4 py-3 text-xs font-black text-red-500 hover:bg-red-50 transition-colors"
                            >
          ...
          <div className="p-8 border-t border-[#F8F9FB] flex items-center justify-between">
            <p className="text-[10px] font-black text-[#94A3B8] uppercase tracking-widest">Menampilkan {processedProducts.length} Produk</p>
            <div className="flex items-center space-x-2">
               <button className="p-2 border border-[#E2E8F0] rounded-lg text-[#94A3B8] opacity-50"><ChevronLeft size={16}/></button>
               <button className="w-8 h-8 rounded-lg bg-[#0047AB] text-white text-xs font-black shadow-lg shadow-blue-100">1</button>
               <button className="p-2 border border-[#E2E8F0] rounded-lg text-[#94A3B8] opacity-50"><ChevronRight size={16}/></button>
            </div>
          </div>
        </div>
      </div>
