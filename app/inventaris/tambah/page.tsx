  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData: any = {
        name: formData.name.toUpperCase(), // Seragamkan nama varian menjadi huruf besar tegas
  ...
  return (
    <div className="text-[#1E293B] ml-0 lg:ml-72 min-h-screen bg-[#F8F9FB] transition-all duration-300 pb-20">
      ...
      <div className="px-4 sm:px-10 pt-6 sm:pt-8">
        <button 
          onClick={() => router.back()}
          className="flex items-center gap-2.5 text-[#64748B] hover:text-[#0047AB] transition-colors font-black text-xs uppercase tracking-wider group"
        >
          {/* 🚀 PERBAIKAN: Memangkas jarak ml-14 luar, ikon dan teks nempel manis bersahabat */}
      ...
      {/* 🚀 PERBAIKAN LAYOUT DESKTOP: Mengubah grid menjadi 12 kolom untuk memanfaatkan area kanan kosong */}
      <div className="px-4 sm:px-10 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
        
        {/* ======================================================= */}
        {/* 📋 SISI KELI: AREA UTAMA FORMULIR INPUT (8 KOLOM DESKTOP)  */}
        {/* ======================================================= */}
        <div className="lg:col-span-8 space-y-5 bg-white p-5 sm:p-8 rounded-[28px] border border-[#F1F5F9] shadow-xs">
          ...
          <div className="pt-2 border-t border-slate-100">
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center space-x-2 bg-[#0047AB] text-white py-4 rounded-xl font-black text-xs uppercase shadow-md shadow-blue-100 hover:bg-[#003580] transition-all disabled:opacity-70 active:scale-98">
