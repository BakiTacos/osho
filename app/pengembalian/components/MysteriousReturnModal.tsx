      ...
      <div className="bg-white w-full max-w-md rounded-[32px] p-6 relative shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="mb-4 flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold text-slate-400 uppercase">Resi hasil scan tidak terdaftar di sistem jualan</p>
          </div>
        </div>
        
        <form onSubmit={onSubmit} className="space-y-3">
          <div>
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Nomor Resi / Pesanan Hasil Scan</label>
            <input type="text" required value={form.orderIdOrResi} onChange={(e) => setForm({...form, orderIdOrResi: e.target.value})} className="w-full bg-slate-100 border border-slate-200 rounded-xl py-2.5 px-3 font-black text-slate-700 text-xs mt-1 outline-none uppercase" placeholder="No Resi Paket"/>
          </div>
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input list="sku-mystery-list" required value={form.sku} onChange={(e) => setForm({...form, sku: e.target.value})} className="w-full bg-slate-50 border border-slate-100 rounded-xl py-2.5 px-3 font-black text-[#0F172A] text-xs mt-1 uppercase outline-none" placeholder="Cari SKU..."/>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="col-span-2">
              <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Catatan Tambahan Paket</label>
              <input required value={form.reason} onChange={(e) => setForm({...form, reason: e.target.value})} className="w-full bg-slate-50 border border-slate-100 rounded-xl py-2.5 px-3 font-bold text-[#0F172A] text-xs mt-1 outline-none" placeholder="Contoh: Label sobek, nama samar"/>
            </div>
          </div>
          <button type="submit" className="w-full bg-[#0047AB] text-white py-3 rounded-xl font-black text-xs uppercase shadow-xs mt-2">Daftarkan Paket Misterius</button>
        </form>
      </div>
    </div>
  );
